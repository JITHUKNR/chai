...
async def regenerate_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in last_user_message or user_id not in chat_history:
        return await query.answer("Cannot regenerate.", show_alert=True)
...
async def date_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Date Handler FAILED: {e}", exc_info=True)
...
async def sub_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cat_video":
        text, btns = "🎥 Video Studio \nCreate cinematic AI videos. ", [
            ...
            [InlineKeyboardButton(f"📸 Image to Video ({PRICE['img2vid']}c)", callback_data="tool_img2vid")],
        ...
        text, btns = "⚙️ Kling AI Motion Control ", [
            [InlineKeyboardButton("🔍 Zoom In/Out", callback_data="motion_zoom"), InlineKeyboardButton("↔️ Pan L/R", callback_data="motion_pan")],
            [InlineKeyboardButton("↕️ Tilt Up/Down", callback_data="motion_tilt"), InlineKeyboardButton("🔄 Roll", callback_data="motion_roll")],
            [InlineKeyboardButton("🔙 Back", callback_data="open_tools")]
        ]
    elif data == "cat_face":
        text, btns = "🎭 Face & Persona ", [
            [InlineKeyboardButton(f"🔄 Face Swap ({PRICE['faceswap']}c)", callback_data="tool_faceswap")],
            [InlineKeyboardButton("🔙 Back", callback_data="open_tools")]
        ]
    elif data == "cat_image":
        text, btns = "🖼️ Image Lab ", [
...
# ---------------------------------------------------------
# 🎨 OLD IMAGINE COMMAND (Replaced by Step-by-Step, but kept)
# ---------------------------------------------------------
async def imagine_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_query = " ".join(context.args)
    if not user_query:
        return await update.message.reply_text("What should I search for? (Example: `/imagine Jungkook cute`) 💜 ")
...
async def send_new_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not establish_db_connection():
        return await message_obj.reply_text("DB Error. ")
    user_doc = db_collection_users.find_one({'user_id': user_id})
    if user_doc and user_doc.get('allow_media') is False:
        return await message_obj.reply_text("Media disabled. ")
    ...
    if cooldown_doc:
        ...
        if elapsed.total_seconds() < COOLDOWN_TIME_SECONDS:
            return await message_obj.reply_text("Wait a bit, darling. 😉 ")
...
async def send_fake_status(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        try:
            await context.bot.send_photo(chat_id=user['user_id'], photo=image_url, caption=f"📸 New Status Update:\n\n{scenario['caption']}", parse_mode='Markdown')
        except Exception: pass
...
async def broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if establish_db_connection():
        ...
        for i in range(0, total_users, batch_size):
            ...
            if i % 100 == 0:
                try:
                    await status_msg.edit_text(f"🚀 Sending... {sent_count}/{total_users}")
                except: pass
...
async def send_morning_wish(context: ContextTypes.DEFAULT_TYPE):
    if establish_db_connection():
        for user in db_collection_users.find({}, {'user_id': 1}):
            try:
                await context.bot.send_message(user['user_id'], "Good Morning, My Love! ☀️❤️ Have a beautiful day!", parse_mode='Markdown')
            except: pass
...
async def check_inactivity(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        try:
            ...
            logger.error(f"Inactivity check failed for user {user['user_id']}: {e}", exc_info=True)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔴 CRITICAL FIX: Ensure user_text is not None
    if not user_text:
        logger.warning(f"Empty text message from user {user_id}")
        return await update.message.reply_text("⚠️ Please send a proper message.")
